...
//	
/////////////////////////////////////////////////////////////////////////////////////////	
	public static <F extends SearchFilter> SearchFilterAsCriteriaString searchFilterToCriteriaString(final F filter) {
		// The filter as as a criteria string has the following structure:
		//		uiLanguage:es#filterType:MySearchFilterType#{boolean query as criteriaString}
		//			[1]      #                  [2]        #               [3]                         
		
		// [1] - Compose the uiLanguage part
		String uiLangPart = filter.getUILanguage() != null ? Strings.customized("uiLanguage:{}",
																			  	filter.getUILanguage()) 
											    		 : null;
		// [2] - Compose the filter's type part
		String filterTypePart = Strings.customized("filterType:{}",
												   filter.getClass().getCanonicalName());
		
		// [3] - Compose the boolean query criteria string
		String boolClausesPart = filter.getBooleanQuery()
									   .encodeAsString();
		// --- Join them all
		...
		if (uiLangPart != null && boolClausesPart != null) {
			...
			outCriteriaStr = filterTypePart;
		}
		return SearchFilterAsCriteriaString.of(outCriteriaStr);
	}
    public static <F extends SearchFilterForModelObject> SearchFilterAsCriteriaString searchFilterForModelObjectToCriteriaString(final F filter,
    																													  		 final HasTypesMetaData hasTypesMetaData) {
		// The filter as as a criteria string has the following structure:
...
/////////////////////////////////////////////////////////////////////////////////////////    
	...
	public static <F extends SearchFilterForModelObject> F searchFilterForModelObjectFromCriteriaString(final SearchFilterAsCriteriaString criteriaStr,
																										final HasTypesMetaData hasTypesMetaData) {
    	F outFilter = null;
		if (!criteriaStr.hasData()) {
			...
			if (m.find()) {
				...
				for (String typeCodeStr : modelObjTypeCodes) {
					...
					TypeMetaData<? extends MetaDataDescribable> modelObjMetaData = hasTypesMetaData.getTypeMetaDataFor(typeCode);
				...
				if (Strings.isNOTNullOrEmpty(clausesStr)) {
					...
					TypeMetaData<? extends MetaDataDescribable> modelObjMetaData = hasTypesMetaData.getTypeMetaDataFor(anyModelObjType);
